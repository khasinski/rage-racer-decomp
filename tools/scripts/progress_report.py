...
def count_functions(path: Path) -> int:
    ...
    definitions = {
        ...
        if not ("static" in match.group(0) and "inline" in match.group(0))
    }
    return max(1, len(INCLUDE_ASM_RE.findall(text)) + len(definitions))
