...
def main() -> int:
    ...
    # Some GNU ld versions evaluate supplemental scripts before resolving an
    # otherwise-defined object symbol.  Put a non-overriding PROVIDE before
    # those scripts so their expressions can still be evaluated.  The real
    # object definition wins, preserving its relocatability.
    provided = script_dependencies & defined - assigned
    required = unresolved | (script_dependencies - assigned)
    ...
    # A linker-script reference alone does not count as a reference for
    # PROVIDE.  EXTERN makes GNU ld consider the fallback while still allowing
    # the definition from the selected object to take precedence.
    out += [f"EXTERN({sym});" for sym, _ in pins if sym in provided]
    out += [
        (f"PROVIDE({sym} = 0x{addr:08X});" if sym in provided else f"{sym} = 0x{addr:08X};")
        for sym, addr in sorted(pins, key=lambda i: i[1])
    ]
