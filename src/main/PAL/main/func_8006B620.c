...
static inline void setAlarm(char *name) {
    D_8009BB08.deadline = func_8006DD30(-1) + 0x3C0;
    D_8009BB08.count = 0;
    D_8009BB08.name = name;
}

static inline s32 getAlarm(void) {
    if (D_8009BB08.deadline < func_8006DD30(-1) ||
        D_8009BB08.count++ > 0x3C0000) {
        func_80063C38(D_80013814);
        func_8001674C(D_80013824, D_8009BB08.name, D_80099060[D_8009905D],
...
s32 CD_cw(u8 command, u8 *params, u8 *result, s32 async) {
    ...
    if (D_80099048 >= 2) {
        func_8001674C(D_800138B0, D_80099060[command]);
    }

    if (D_80099280[command] != 0 && params == 0) {
        if (D_80099048 > 0) {
            func_8001674C(D_800138B8, D_80099060[command]);
        }
        return -2;
    }
