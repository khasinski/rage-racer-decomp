...
void UpdatePlayerCar(PlayerCarRuntime *car) {
    ...
    if (car->shiftState == 0) {
        ...
        if (spd < 256 && p->motionState == CAR_MOTION_DRIVING) {
            p->targetHeading += (p->steerPos * p->steeringGrip / 256) * spd / 0x10000;
        } else if (spd < 512 && p->motionState == CAR_MOTION_STANDING_START) {
            p->targetHeading += (p->steerPos * p->steeringGrip / 256) * spd / 0x20000;
        } else {
            p->targetHeading += p->steerPos * p->steeringGrip / 0x10000;
        }
    }
    ...
    tmp.x = (p->accelPos / 256) + car->x + car->motionX;
    tmp.z = (p->brakePos / 256) + car->z + car->motionZ;
    ...
    if (car->shiftState != 0) {
        ...
        if (car->shiftState == 1) {
            ...
            car->y = car->shiftRef * t + (t * t * 60) / 100 + car->y;
            if (car->y >= limit) {
                car->shiftState = 0;
            }
            ...
            car->y = car->shiftBase + (n * n * 180) / 100;
            if (car->y >= limit) {
                car->shiftState = 0;
            }
        }
