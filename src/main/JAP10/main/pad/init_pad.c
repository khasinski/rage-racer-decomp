...
/* BIOS InitPAD over the two pad buffers, then StartPAD. The 1.0 release
 * hands the BIOS 0x10-byte buffers; 1.1 grew them to the 0x28-byte layout
 * every later build keeps. */
void GameInitPad(void) {
    InitPad(g_PadBuffers, 0x10, g_PadBuffers + 0x10, 0x10);
    StartPad();
}
